...
times 446-($-$$) db 0 ;中间的填0,然后来划分下分区;就两个一个特殊的系统分区，另一个可以弄成文件系统,不弄扩展分区
;这里不算CHS的相关数据，后面自己解析也只用LBA的数据
;C.img 大概是60MB loader和系统代码划分个10MB 剩下的50MB当文件系统用
;fdisk看C.img总共是121968个扇区，根据这个计算
;有引导标志，起始扇区是0,总共20480个扇区
db 0x80, 0x00, 0x00, 0x00, 0x83, 0x00, 0x00, 0x00,   0x00,0x00,0x00,0x00,  0x00,0x50,0x00,0x00
;中间隔个256个扇区 最后也隔开个256当缓冲区吧
db 0x00, 0x00, 0x00, 0x00, 0x83, 0x00, 0x00, 0x00,   0x00,0x51,0x00,0x00,   0x70,0x8A,0x01,0x00
